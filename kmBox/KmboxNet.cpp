...
{
    if (rx->head.cmd != tx->head.cmd) return err_net_cmd;           // 命令码错误
    if (rx->head.indexpts != tx->head.indexpts) return err_net_pts; // 时间戳错误
    return 0;                                                       // 没有错误返回0
...
#include <WS2tcpip.h>
    ...
    timeout.tv_usec = 0; // 毫秒部分设置为 0
    if (setsockopt(sockClientfd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) < 0) {
        std::cerr << "Failed to set socket receive timeout" << std::endl;
        close(sockClientfd);
        return 1;
    }
...
int kmNet_keydown(int vk_key) {
    ...
    if (vk_key >= KEY_LEFTCONTROL && vk_key <= KEY_RIGHT_GUI) // 控制键
    {
        ...
        {
            if (softkeyboard.button[i] == vk_key) goto KM_down_send; // 队列里面已经有vk_key 直接发送就行
        }
        // 队列里面没有vk_key
        for (i = 0; i < 10; i++) // 遍历所有的数据，将vk_key添加到队列里
        {
            if (softkeyboard.button[i] == 0) { // 队列里面已经有vk_key 直接发送就行
